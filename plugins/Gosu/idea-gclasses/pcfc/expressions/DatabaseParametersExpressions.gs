...
class DatabaseParametersExpressions {
  ...
  public static class ScreenExpressionsImpl extends DatabaseParametersExpressionsImpl {
    ...
    // 'action' attribute on ButtonInput (id=RefreshParametersButton_Input) at DatabaseParameters.pcf: line 40, column 115
    function action_3 () : void {
      ParameterSets = gw.api.tools.DatabaseParametersHelper.getParameters()
    }
    
    // 'action' attribute on ButtonInput (id=DownloadParms_Input) at DatabaseParameters.pcf: line 46, column 106
    function action_4 () : void {
      gw.api.tools.DatabaseParametersHelper.download()
    }
    
    // 'def' attribute on PanelRef at DatabaseParameters.pcf: line 60, column 122
    function def_onEnter_11 (def :  pcf.DatabaseParametersLV) : void {
      def.onEnter(gw.api.tools.DatabaseParametersHelper.getSetByName( ParameterSetName, ParameterSets))
    }
    
    // 'def' attribute on PanelRef at DatabaseParameters.pcf: line 60, column 122
    function def_refreshVariables_12 (def :  pcf.DatabaseParametersLV) : void {
      def.refreshVariables(gw.api.tools.DatabaseParametersHelper.getSetByName( ParameterSetName, ParameterSets))
    }
    
    // 'value' attribute on RangeInput (id=paramSetPicker_Input) at DatabaseParameters.pcf: line 54, column 42
    ...
    // 'valueRange' attribute on RangeInput (id=paramSetPicker_Input) at DatabaseParameters.pcf: line 54, column 42
    ...
    // 'visible' attribute on Verbatim at DatabaseParameters.pcf: line 30, column 46
